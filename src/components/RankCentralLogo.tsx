...
export const RankCentralLogo: React.FC<{ className?: string; size?: number }> = ({ 
  className = "", 
  size = 35  // Made logo slightly smaller
}) => {
  return (
    <div className={`relative flex items-center ${className}`}>
      ...
      <span className="font-bold text-brand-primary text-3xl ml-1">
        rank<span className="text-brand-accent text-3xl">Central</span>
      </span>
    </div>
  );
};
